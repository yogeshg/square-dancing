...
class Simulator {
	...
	public static void main(String[] args) {
		...
		try {
			for (int a = 0; a != args.length; ++a)
				if (args[a].equals("-f") || args[a].equals("--friends")) {
					...
					throw new IllegalArgumentException("Unknown argument: " + args[a]);
		...
		int max_score = -1;
		try {
			max_score = game(group, player_class, friends, strangers, room_side, turns, score, met_soulmate, gui,
					gui_refresh, verbose);
		} catch (Exception e) {
		...
		for (int i = 0; i != score.length; ++i) {
			out.println("Player " + i + " (" + group + ") scored: " + score[i]
	...
	private static int game(String group, Class<Player> player_class, int friends, int strangers, int room_side,
			int turns, int[] score, boolean[] met_soulmate, boolean gui, long gui_refresh, boolean verbose)
					throws Exception {
		...
		Point[] sL = null;
		Point[] L = null;
		Point[] Lp = new Point[N]; // previous location of player, for drawing
									// movement lines
		...
		for (int turn = 0; turn != turns; ++turn) {
			...
			// GUI state
			if (gui)
				gui(server, state(group, L, Lp, score, max_score, W, C, F, Sm, room_side, clock, gui_refresh));
			if (out != null)
				println(out, clock);
			...
			try {
				...
				if (e instanceof TimeoutException) {
					System.err.println(group + " timed out during \"play\"!");
					System.exit(1);
				} else {
					e.printStackTrace();
					System.exit(1);
			...
			for (int i = 0; i != N; ++i)
				if (M[i].id == i) {
					println(out, i + " moved from (" + L[i].x + ", " + L[i].y + ")" + " to (" + (L[i].x + M[i].x) + ", "
							+ (L[i].y + M[i].y) + ")");
				}
			...
			for (int i = 0; i != N; ++i) {
				...
				if (i > j && M[j].id == i) // avoid double processing dance
											// pairs, only do when i < j
					continue;
				if (Sm[i] == j) {
					met_soulmate[i] = true;
					met_soulmate[j] = true;
				...
				if (W[j][i] == 0 && c) { // still dance with each other but not
											// enjoying it
					L[i].id = j;
					L[j].id = i;
					println(out, i + " and " + j + " do not enjoy dancing with each other anymore.");
				} else if (!c) { // cannot dance because of physical
									// obstructions
					L[i].id = i;
					L[j].id = j;
		...
		if (gui) {
			gui(server, state(group, L, Lp, score, max_score, W, C, F, Sm, room_side, clock(turns * 6), -1));
			server.close();
		}
		return max_score;
	}
	...
	private static String clock(int seconds) {
		...
		return hour + ":" + minutes_hi + "" + minutes_lo + ":" + seconds_hi + "" + seconds_lo;
	}
		...
		for (int d = 0; d != degree; ++d) {
			...
			for (int i = 0; i != nodes; ++i) {
				...
				if (k == 0) {
					...
					nested_loop: for (i2 = 0; i2 != nodes; ++i2) {
						if (!M[i2])
							continue;
						for (j2 = 0; j2 != nodes; ++j2) {
							if (!M[j2] || !C[i2][j2])
	...
	private static String state(String group, Point[] locations, Point[] previous_locations, int[] score, int max_score,
			int[][] wisdom, boolean[] wiser, boolean[][] friends, int[] soulmates, int side, String clock,
			long gui_refresh) {
		int N = locations.length;
		StringBuffer buf = new StringBuffer();
		buf.append(N + "," + side + "," + clock + "," + max_score + "," + gui_refresh);
		for (int i = 0; i != N; ++i) {
			int j = locations[i].id;
			buf.append("," + group + "," + locations[i].x + "," + locations[i].y + "," + previous_locations[i].x + ","
					+ previous_locations[i].y + "," + j + "," + (wiser[i] ? 1 : 0) + "," + wisdom[j][i] + ","
					+ (friends[i][j] ? 1 : (j == soulmates[i] ? 2 : 0)) + "," + score[i]);
		}
		return buf.toString();
	}

	private static void gui(HTTPServer server, String content) throws UnknownServiceException {
		String path = null;
		for (;;) {
			// get request
			for (;;)
			...
			if (path.equals(""))
				path = "webpage.html";
			else if (!path.equals("favicon.ico") && !path.equals("apple-touch-icon.png") && !path.equals("script.js"))
				break;
			// send file
			File file = new File(root + File.separator + "sim" + File.separator + path);
			try {
				server.reply(file);
			} catch (IOException e) {
	...
	private static Class<Player> load(String group) throws IOException, ReflectiveOperationException {
		...
		if (class_modified < 0 || class_modified < last_modified(player_files)
			...
			if (!compiler.getTask(null, manager, null, null, null, manager.getJavaFileObjectsFromFiles(player_files))
					.call())
